...
class IMAPSensor(PollingSensor):
    ...
    def _parse_accounts(self, accounts):
        for config in accounts:
            self._logger.debug('[IMAPSensor]: entering accounts')
            mailbox = config.get('name', 'Mukesh123')
            ...
            self._logger.debug('[IMAPSensor]: entering connecting')
    ...
    def _poll_for_unread_messages(self, name, mailbox, mailbox_metadata):
        ...
        self._logger.debug('[IMAPSensor]:messages')
    ...
    def _process_message(self, uid, mailbox, mailbox_metadata):
        ...
        self._logger.debug('[IMAPSensor]:process_message')
        print(message)
        ...
        self._logger.debug('[IMAPSensor]:Before Dispatch')
    ...
    def _flattern_headers(self, headers):
        ...
        result = []
        self._logger.debug('[IMAPSensor]:headers')
        for pair in headers:
            name = pair[0]
            value = pair[1]
    ...
    #         self.sensor_service.dispatch(trigger='azure_vm_create.rule1', payload=payload)
